...
for (s in c("LUG", "RIG")){
  path <- paste('./',s,'_hourly_1.png', sep='')
  png(path, width=400, height=700)
  par(mfrow = c(4,2))
  for (pol in pollutants[0:4]){
    ...
    qqcomp(fit, main = paste('Q-Q Plot For', pol,'at', s))
  }
  dev.off()
  
  path <- paste('./',s,'_hourly_2.png', sep='')
  png(path, width=400, height=700)
  par(mfrow = c(4,2))
  for (pol in pollutants[5:length(pollutants)]){
    df1 <- july.hour %>% filter(variable == pol & site == s)
    concvec <- c(na.omit(df1)[["value"]])
    fit <- fitdist(ReplaceNonpositive(concvec), "lnorm")
    fit_stats <- gofstat(fit)
...
for (s in c("LUG", "RIG")){
  path <- paste('./',s,'_daily_1.png', sep='')
  png(path, width=400, height=700)
  par(mfrow = c(4,2))
  for (pol in pollutants[0:4]){
    df1 <- july.day %>% filter(variable == pol & site == s)
    concvec <- c(na.omit(df1)[["value"]])
    fit <- fitdist(ReplaceNonpositive(concvec), "lnorm")
    fit_stats <- gofstat(fit)
...
# Below kept to show how results were obtained previously
# 
